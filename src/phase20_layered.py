"""
NOMS-LAB connectome-gen : 단계 XX-L — 층구조 대용량 뇌 생성 (~300GB) → D

phase20 + #7 피질 층구조: 뉴런에 깊이 부여 → 깊이별 타입분포(mouse_layers)로 층 배치.
공간-국소 희소 스트리밍, 청크마다 D에 직접 기록. (src u32, dst u32, w u16)=10B.
사용: py -3.12 src/phase20_layered.py [target_GB] [N]
"""
import os, sys, time
import numpy as np
import torch
...
LAYERS = os.path.join(HERE, "..", "data", "processed", "mouse_layers.npz")
...
CHUNK = 32_000
C = 128
OUTFILE = os.path.join(DOUT, f"bigbrain_{int(TARGET_GB)}gb_layered.bin")
...
def train_rule():
    ...
    return model, T, float(dscale), Nm / span.prod().item()


def main():
    model, T, dscale, dvol = train_rule()
    lay = np.load(LAYERS, allow_pickle=True)
    dtp = torch.tensor(lay["depth_type_prob"], dtype=torch.float32, device=DEV)  # (10,T)
    cum = dtp.cumsum(1)
    target_bytes = TARGET_GB * 1e9
    target_deg = target_bytes / (N * BYTES_PER_EDGE)
    print(f"목표 {TARGET_GB}GB | 뉴런 {N:,} | 층구조 | 차수~{target_deg:.0f} | {OUTFILE}", flush=True)

    side = ((N / dvol) ** (1 / 3)) / dscale
    print("층구조 배치 중...", flush=True)
    depth = torch.rand(N, device=DEV)                          # 0~1 피질 깊이
    pos = torch.empty(N, 3, device=DEV)
    pos[:, 0] = depth * side
    ...
    f = open(OUTFILE, "wb"); written = 0; n_edges = 0; t0 = time.time(); last = t0
    for p0 in range(0, N, CHUNK):
        ...
        with torch.no_grad():
            dist = (pos[si].float() - pos[cand].float()).norm(dim=-1, keepdim=True)
        ...
        if time.time() - last > 30:
            ...
            print(f"  {gb:.1f}/{TARGET_GB}GB | {n_edges:,}엣지 | {rate*60:.1f}GB/분 | ETA {(TARGET_GB-gb)/max(rate,1e-9)/60:.0f}분", flush=True)
            last = time.time()
        if written >= target_bytes:
            break
    f.close()
    dt = time.time() - t0
    np.savez(os.path.join(DOUT, f"bigbrain_{int(TARGET_GB)}gb_layered_meta.npz"),
             N=N, edges=n_edges, bytes=written, T=T, seconds=dt, layered=True)
    print(f"\n완료: {written/1e9:.1f}GB, {n_edges:,}엣지, {N:,}뉴런(층구조), {dt/60:.1f}분", flush=True)
